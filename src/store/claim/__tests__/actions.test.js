...
describe('transformClaimSubmit', () => {
  ...
  const expected = {
    ...
    contactNumber: '81238789',
    dischargeDate: '2019-04-01T00:00:00',
...
describe('GetCXA1ClaimFilters', () => {
  test('should create action to get cxa1 claim filters', async () => {
    const store = configureMockStore(api)({
      claim: { filters: {} },
      user: {
        clientId: 'clientId',
    ...
    const expectedActions = [
      { type: types.GET_CXA1_CLAIM_FILTERS_START },
      {
    ...
    await store.dispatch(actions.getCXA1ClaimFilters());
    expect(store.getActions()).toEqual(expectedActions);
  });
});
...
describe('updateCXA1ClaimFilters', () => {
  it('should update claim list filters', async () => {
    const store = configureMockStore(api)({
      claim: { filters: {} },
      user: {
        clientId: 'clientId',
    ...
    const expectedActions = [{ type: types.UPDATE_CXA1_CLAIM_FILTERS }];

    await store.dispatch(actions.updateCXA1ClaimFilters());
    expect(store.getActions()).toEqual(expectedActions);
  });
});
...
describe('call getClaims api by filters', () => {
  ...
  it('should call getClaims with filter REQUEST FOR INFORMATION if PROCESSING selected', async () => {
    ...
    expect(filterClaimsapi.getClaims).toHaveBeenCalledWith(
      'clientId',
      'userId',
      'statuses=PROCESSING,REQUEST FOR INFORMATION&categoryCodes=outpatient',
    );
  });
...
describe('Fetch cxa1 claims data for claim list screen', () => {
  test('should create action to get cxa1 claims', async () => {
    const store = configureMockStore(api)({
      claim: { selectedCXA1ClaimFilters: [], claimsMap: {} },
      user: {
        clientId: 'clientId',
        userId: 'userId',
      },
    ...
    await store.dispatch(actions.getCXA1Claims());
    expect(store.getActions()).toEqual(expectedActions);
  });
});

describe('call getCXA1Claims api by filters', () => {
  const filterClaimsapi = {
    getClaims: jest.fn(() => ({
      data: [
        {
  ...
  it('should call getClaims with filter PROCESSING selected', async () => {
    const store = configureMockStore(filterClaimsapi)({
      claim: {
        selectedCXA1ClaimFilters: [
          { type: 'claimStatusFilters', value: 'PROCESSING' },
          { type: 'claimCategoryFilters', value: 'flex' },
          { type: 'claimPlanFilters', value: '2020' },
        ],
        claimsMap: {},
      },
      user: {
    ...
    expect(filterClaimsapi.getClaims).toHaveBeenCalledWith(
      'clientId',
      'userId',
      'statuses=PROCESSING,REQUEST FOR INFORMATION&planIds=2020&categoryCodes=flex',
    );
  });

  it('should call getClaims with filter REJECTED selected', async () => {
    const store = configureMockStore(filterClaimsapi)({
      claim: {
        selectedCXA1ClaimFilters: [
          { type: 'claimStatusFilters', value: 'REJECTED' },
