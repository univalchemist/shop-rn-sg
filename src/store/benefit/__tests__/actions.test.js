...
describe('Benefit actions', () => {
  ...
  it('should create an action to fetch benefit simplified', () => {
    const initialState = {
      user: {
        data: {
          clientId: 'testClient',
    ...
    const store = configureMockStore(api)(initialState);
    const expectedActions = [
      { type: types.FETCH_CXA1_BENEFITS_SIMPLIFIED_START },
      {
    ...
    return store.dispatch(actions.fetchCxa1BenefitsSimplified()).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
});
