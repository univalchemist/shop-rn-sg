...
describe('Shop config actions', () => {
  it('should create an action to get home data', () => {
    const initialState = {
      ...
      shop: {
        paginations: { suggestedProducts: { page: 1 } },
      },
    };
    ...
    return store.dispatch(actions.getSuggestedOffers()).then(() => {
  ...
  it('should create an action to get products by search term', () => {
    const initialState = {
      user: { clientId: 'cxadev' },
      shop: {
        filters: { params: { categoryId: [1, 2], searchString: 'medicine' } },
      },
    };
    const store = configureMockStore(api)(initialState);
    const expectedActions = [
      {
        type: types.GET_PRODUCTS_BY_SEARCH_TERM_START,
      },
      {
    ...
    return store.dispatch(actions.getProductBySearchTerm()).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
