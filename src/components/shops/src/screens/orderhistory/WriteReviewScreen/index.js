...
import { getLocalizeServerError } from '@shops/utils';
  ...
  const handleSubmit = async reviewData => {
    ...
    try {
      ...
      const { subject, message } = getLocalizeServerError(
        error,
        {
          subjectPrefix: 'shop.serverErrors.sendReview.subject',
          prefix: 'shop.serverErrors.sendReview',
        },
        intl,
      );
        ...
        <Box flex={1} flexDirection="row">
          <Box flex={1}>
            {isReviewing ? (
              ...
              <TrackedButton
                primary
                disabled={!isAcceptRule || isLoading}
                onPress={() => {
