  ...
  return (
    ...
    >
      {type === ADD_TO_CART_SUCCESS ? (
        ...
        type === ADD_TO_CART_FAILED && <AddToCartFailedView {...props} />
      )}
    </TouchableOpacity>
  );
};
    ...
    >
      ...
      >
        ...
        <Text color={theme.colors.white}>
          {intl.formatMessage(
            {
              id: 'shop.product.snackBarMessageSuccess',
              defaultMessage: `${quantity} items added to cart.`,
            },
            {
              quantity,
            },
...
export const AddToCartFailedView = ({ onCancelPress, text }) => {
  const theme = useTheme();
  const intl = useIntl();
  return (
    <Box
    ...
    >
      <Box minHeight={80} flexDirection="row" alignItems="center" width="80%">
        <Box mx={20} mt={-2}>
          <Icon
            name="error-outline"
            fontSize={20}
