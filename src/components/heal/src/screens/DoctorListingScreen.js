...
import { HEAL_UNIFY_SEARCH, HEAL_DOCTOR_DETAIL } from '@heal/routes';
  ...
  return (
    ...
    >
      ...
      <NonTouchableSearchBar
        onPress={() => navigation.navigate(HEAL_UNIFY_SEARCH)}
        mr={34}
        placeholder={intl.formatMessage({
          id: 'heal.doctorSearch.placeholder',
        })}
      />
    </Flex>
  );
    ...
    <TouchableOpacity onPress={onPress}>
      ...
      >
        <Box>
          ...
          <Text mb={8}>
            {intl.formatMessage({
              id: `heal.speciality.name.${item.specialityCode}`,
            })}
          </Text>
          ...
          <FlatList
            ...
            ListHeaderComponent={() => (
              <SectionHeadingText mt={24} mb={1}>
                {intl.formatMessage({ id: 'heal.doctor.found' }, { total: total })}
              </SectionHeadingText>
            )}
            ...
            renderItem={({ item, index }) => (
              <ListItem
                ...
                item={item}
                onPress={() =>
                  navigation.navigate(HEAL_DOCTOR_DETAIL, { doctor: item })
                }
              />
            )}
