...
import { HEAL_CLINIC_DETAILS } from '@heal/routes';
  ...
  return (
    ...
    >
      ...
      >
        {total === 1
          ? intl.formatMessage(
              { id: 'heal.clinic.singularFound' },
              { number: total },
            )
          : intl.formatMessage({ id: 'heal.clinic.pluralFound' }, { number: total })}
      </Text>
    </Box>
  );
};
...
const ClinicListing = ({
          ...
          return (
            <ClinicListingItem
              ...
              onPress={() => {
                ...
                navigation.navigate(HEAL_CLINIC_DETAILS, {
                  clinic: item,
                });
              }}
            />
  ...
  return (
    <Box flex={1}>
      <ErrorPanel
        heading={intl.formatMessage({ id: 'heal.clinic.noClinics' })}
        description={intl.formatMessage({ id: 'heal.clinic.noClinicsDescription' })}
      />
    </Box>
  );
};
