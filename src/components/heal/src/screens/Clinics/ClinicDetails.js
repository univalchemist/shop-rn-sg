...
import { getDoctorsInfo, resetVisitList } from '@heal/src/store/actions';
import { HEAL_CLINIC_INFORMATION } from '@heal/routes';
      ...
      if (rawArray[i] !== '') {
        ...
        const dayValue = value[1]
          .replace(
            ...
            intl.formatMessage({ id: 'heal.clinic.closed' }),
          )
          .replace('00:00-00:00', intl.formatMessage({ id: 'heal.clinic.closed' }));
  ...
  return (
    <Box flex={1} as={SafeAreaView}>
      <ScrollView>
        ...
        <MarginBox>
          <PlainText>{intl.formatMessage({ id: 'heal.clinic.doctors' })}</PlainText>
        </MarginBox>
        <TrackedCarousel
          useScrollView
        ...
        <MarginBox>
          <PlainText color={theme.colors.gray[1]}>
            {intl.formatMessage({ id: 'heal.clinic.clinicStatus' })}
          </PlainText>
          <PlainText style={styles.status} color={theme.colors.gray[0]}>
            ...
            detailsClinic.doctors[currentDoctorIndex].isOpen
              ? intl.formatMessage({ id: 'heal.clinic.open' })
              : intl.formatMessage({ id: 'heal.clinic.closed' })}
          </PlainText>
        </MarginBox>
        {detailsClinic.openingHours ? (
          <React.Fragment>
            <Divider />
            <MarginBox>
                ...
                return (
                  <DayText
                    title={
                      dayValue.label
                        ? intl.formatMessage({
                            id: `heal.clinic.openingHours.${dayValue.label}`,
                          })
                        : ''
                    }
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          resetVisitList();
          navigation.navigate(HEAL_CLINIC_INFORMATION, {
...
export default connect(mapStateToProps, { getDoctorsInfo, resetVisitList })(
  ClinicDetails,
);
