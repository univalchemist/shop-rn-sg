...
import { HEAL_APPOINTMENT_REQUESTED } from '@heal/routes';
...
const SelectClinic = ({ route, navigation, requestAppointment }) => {
  ...
  const dependentId = patient.isDependent && patient._id;
  ...
  const renderAlert = index => {
    Alert.alert(
      intl.formatMessage({
        id: 'heal.doctor.scheduleAppointment.alert',
      ...
      [
        {
          text: intl.formatMessage({
            id: 'heal.doctor.scheduleAppointment.delete',
        ...
        {
          text: intl.formatMessage({
            id: 'heal.doctor.scheduleAppointment.cancel',
    ...
    <Box flex={1} as={SafeAreaView} backgroundColor={theme.colors.white}>
      <ScrollView backgroundColor={theme.colors.white}>
        <Box px={32} pt={32}>
          <SectionHeadingText>
            {intl.formatMessage({
              id: 'heal.doctor.scheduleAppointment.selectClinic',
            })}
          </SectionHeadingText>
        </Box>
        <Box pt={32} px={32}>
        ...
        <Box pl={32}>
          <SectionHeadingText>
            {intl.formatMessage({
              id: 'heal.doctor.scheduleAppointment.pickDates',
            })}
          </SectionHeadingText>
          <PlainText>
            {intl.formatMessage({
              id: 'heal.doctor.scheduleAppointment.maxDates',
            })}
          </PlainText>
        </Box>
        <Box p={32}>
          ...
          {addedDates.length < 3 && (
            <CoreButton
              ...
              title={intl.formatMessage({
                id: 'heal.doctor.scheduleAppointment.addDates',
              })}
            />
          )}
          <DateSelector
      ...
      <Button
        ...
        title={intl.formatMessage({
          id: 'heal.doctor.scheduleAppointment.requestAppointment',
        })}
      />
    </Box>
  );
