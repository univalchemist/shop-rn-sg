...
import { HEAL_SELECT_CLINIC } from '@heal/routes';
  ...
  return (
    ...
    >
      <Box flex={1} p={32}>
        ...
        >
          {intl.formatMessage({
            id: 'heal.doctor.scheduleAppointment.selectFamilyMember',
          })}
        </SectionHeadingText>
        <PlainText color={theme.colors.gray[8]} mt={16}>
          {intl.formatMessage({
            id: 'heal.doctor.scheduleAppointment.confirmation',
          })}
        </PlainText>
        <Box py={45}>
      ...
      <Button
        onPress={() =>
          navigation.navigate(HEAL_SELECT_CLINIC, {
            doctor: doctor,
            patient: familyMember.filter(item => item._id === patient)[0],
          })
        }
        value={patient}
        title={intl.formatMessage({
          id: 'heal.doctor.scheduleAppointment.next',
        })}
      />
    </Box>
  );
