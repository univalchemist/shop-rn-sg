...
import { HEAL_UNIFY_SEARCH, HEAL_DOCTOR_LISTING } from '@heal/routes';
...
export const SpecialistItem = ({ name, code, navigation }) => {
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.7}
      onPress={() => navigation.navigate(HEAL_DOCTOR_LISTING, { code: code })}
    >
      <Card
        width={cardWidth}
        minHeight={56}
      ...
      <FlatList
        ...
        ListHeaderComponent={() => (
          <Box>
            ...
            >
              {intl.formatMessage({ id: 'heal.findSpecialist' })}
            </PlainText>
          </Box>
        )}
        renderItem={({ item }) => (
