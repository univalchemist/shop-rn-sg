...
import { fetchClaimTypes, fetchCXA1ClaimTypes } from '@store/claimType/actions';
...
const ClaimPatientDetailsScreen = ({
  ...
  const [isLoading, isError] = useFetchActions([
    isCXA1Integration ? fetchCXA1ClaimTypes : fetchClaimTypes,
  ]);
  ...
  const validateValidContactNumber = value => {
    if (value?.length > 0) return validateContactNumber(value);
    else return '';
  };
...
const enhance = compose(
  connect(
    ({ user: { userId, membersMap, isCXA1Integration } }) => ({
      initialValues: {
